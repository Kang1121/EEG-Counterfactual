...
def compute_eval_metrics(
    ...
    for counterfactual in counterfactuals.values():
        ...
        q, d  = [], []
        for i in counterfactuals.values():
            for j in i["edits"]:
                q.append(j[0]), d.append(j[1])
        # compute how often each number appear in q
        qq = np.bincount(np.array(q))
        qq = np.append(qq, np.zeros(49 - len(qq))) if len(qq) < 49 else qq
        qq = qq.reshape(7, 7)

        dd = np.bincount(np.array(d) % 49)
        dd = np.append(dd, np.zeros(49 - len(dd))) if len(dd) < 49 else dd
        dd = dd.reshape(7, 7)
