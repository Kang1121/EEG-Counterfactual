...
@torch.no_grad()
def process_dataset(model, dataloader, device, config):
    """
    ...
    """
    top1 = torchmetrics.Accuracy(top_k=1, task="multiclass", num_classes=config['model_kwargs']['num_classes'])


    top1.to(device)
    model.to(device)
    DDP(model, device_ids=[config['local_rank']], output_device=config['local_rank'],
        find_unused_parameters=True) if config['use_ddp'] else None
