...
def _find_single_best_edit(
    ...
    def process_in_chunks(tensor, chunk_size):
        """Divide tensor into chunks of specified size."""
        num_chunks = len(tensor) // chunk_size + (1 if len(tensor) % chunk_size != 0 else 0)
    ...
    for chunk in process_in_chunks(all_combinations, chunk_size):
        logits_chunk = classification_head(
            torch.transpose(chunk, 1, 2)
            .contiguous()
            .view(-1, n_feat, n_pix, n_pix)
            .to(device)
        )
    ...
    logits_class = torch.cat(logits_list, dim=0)
